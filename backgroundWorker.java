...
public class backgroundWorker extends AsyncTask<String, Void, String> {
    ...
    String type;
    AssignWorker a = new AssignWorker();
    ...
    public String doInBackground(String... params) {
        ...
        else if (type.equals("SubmitHours"))
        {
            try {
                ...
                try {
                    ...
                    System.out.println("SEGMENTED: " + result);
        ...
        else if (type.equals("sendWork")) {
                try {
                    String jobType = params[1];
                    String worker = params[2];
                    String location = params[3];
                    URL url = new URL(send_Jobs);
                    HttpURLConnection httpURLConnection = (HttpURLConnection) url.openConnection();
                    httpURLConnection.setRequestMethod("POST");
                    httpURLConnection.setDoOutput(true);
                    httpURLConnection.setDoInput(true);
                    ...
                    BufferedWriter bufferedWriter = new BufferedWriter(new OutputStreamWriter(outputStream, "UTF-8"));
                    String post_data = URLEncoder.encode("JobTipo", "UTF-8") + "=" + URLEncoder.encode(jobType, "UTF-8") + "&"
                            + URLEncoder.encode("assignedWorker", "UTF-8") + "=" + URLEncoder.encode(worker, "UTF-8") + "&"
                            + URLEncoder.encode("workSite", "UTF-8") + "=" + URLEncoder.encode(location, "UTF-8");
                    bufferedWriter.write(post_data);
                    bufferedWriter.flush();
                    bufferedWriter.close();
                    outputStream.close();
    ...
    protected void onPostExecute(String result) {
        ...
        else if(type.equals("SubmitHours"))
        {
            ...
            alertDialog.setMessage(result);
               ...
               a.editStuff(result);
            alertDialog.setTitle("Worker Assigned");
            alertDialog.setMessage(result);
            alertDialog.show();
        }
